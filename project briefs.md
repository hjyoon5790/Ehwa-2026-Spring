...
| 팀번호 | 팀명 | 프로젝트 요약 |
|:------:|------|--------------|
| [1](#team-1) | 이사장님 | 본 연구의 목표는 위성 이미지, 기상 관측 데이터, 수치예보 데이터 등 다양한 모달리티의 기상 데이터를 통합적으로 학습할 수 있는 멀티모달 도메인 적응 기반 날씨 예측 모델을 구축하는 것이다. 이를 통해 서로 다른 지역 및 데이터 수집 환경에서도 안정적으로 동작할 수 있는 domain generalization 능력을 갖춘 기상 예측 시스템을 개발하고자 한다. |
| [2](#team-2) | Sudo | - 실시간 AR 가이드: 카메라로 비추면 화면 위에 "라벨 제거 필요", "이물질 세척 필요" 등의 안내 문구를 증강 현실(AR)로 띄워주는 인터페이스. - 성분 기반 정밀 판별기: OCR로 읽어낸 텍스트를 분석하여 재활용 가능 여부를 즉시 판단(예: "내열유리는 재활용이 안 되니 종량제에 버리세요"). - 그린 마일리지 시스템: 올바르게 배출한 사진을 AI가 최종 인증하면 포인트를 지급하고, 이를 지역 화폐나 편의점 쿠폰으로 교환하는 기능. - 다국어 지원: 영어, 중국어, 베트남어 등 주요 외국어 번역 가이드 제공. |
| [3](#team-3) | 할루캐쳐 | 기존 모델과 온톨로지가 탑재된 모델의 성능(정확도) 비교한 연구논문 |
| [5](#team-5) | 규교굥 | 달을 배경으로 한 액션 어드벤처 2D 게임이다. 달이 치즈로 이루어진 세계관에서 주인공은 달에서 지구로 치즈를 수출하는 회사 <치즈앤더문 주식회사>에 취직한다. 달에 있는 근무지에 파견된 주인공은 그곳에서 채광, 가공, 납품 등의 임무를 수행하여 돈을 번다. 플레이어는 달 배경의 맵 곳곳을 돌아다니며 도구를 이용해 치즈를 수집하고 살상 로봇과 전투하며 기지에서 도구를 업그레이드하거나 AI NPC인 구조 로봇과 소통할 수 있다. |
| [10](#team-10) | 212223 | 시간관리어플 |
| [16](#team-16) | 퓨터 | 소설 보조 어시스턴트 |
| [18](#team-18) | 디바트(deep-art) | 전시 정보를 입력하면 AI가 도슨트 설명을 생성하고 음성으로 변환해주는 AI 기반 오디오 도슨트 자동 제작 서비스 |
| [19](#team-19) | Logue | 자연어 질문만으로 데이터베이스에서 필요한 정보를 조회할 수 있는 데이터 분석 인터페이스 |
